...
public class ThreadPoolFactory {
    ...
    private static final ThreadPoolExecutor daoExecutor = new ThreadPoolExecutor(
            4, 8, 300L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new BasicThreadFactory.Builder().daemon(true)
                    .namingPattern("io-dao-help-worker-%d").build(),
            new ThreadPoolExecutor.CallerRunsPolicy());
    ...
    public static ThreadPoolExecutor getDaoExecutor() {
        return daoExecutor;
    }
    ...
    public void daoExecute(final Runnable task) {
    	daoExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // 防御性容错，避免高并发下的一些问题
                    LOGGER.error("ThreadPoolFactory|daoExecute|Throwable:", th);
                }
